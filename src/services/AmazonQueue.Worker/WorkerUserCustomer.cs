...
{
    public class WorkerUserCustomer : BackgroundService
    {
        private readonly IMessageBusService _busService;

        public WorkerUserCustomer(IMessageBusService busService)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                var queueUrl = await _busService.GetQueueUrl(QueueTypes.AWS_SQS_USER_CREATED_CUSTOMER);
        ...
        private bool ProcessMessage(Message msg)
        {
            ...
            Console.WriteLine($"Recebido Usuario Customer : {user.Id}");
